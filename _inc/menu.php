...
function sitemember_menu(){
	global $sitemember_screen,$sitemember_screen_manage,$_memberLevel;
	add_menu_page('Site Member','Site Member','administrator','site-member','sitemember_ui','');
	...
	$sitemember_screen = add_submenu_page('site-member','Site Members Overview','Overview','administrator','site-member','sitemember_ui');
		...
		foreach($_memberLevel as $key => $val){
		...
		//print_r($sitemember_screen_manage);
		add_submenu_page('site-member','Settings & Config','Settings & Config','administrator','site-member-settings','sitemember_manage_ui');
		
	
	if(isset($_GET['page'])){
		if(strrpos($_GET['page'],'site-member') !== false){
			add_action('admin_print_scripts','sitemember_action_script');
		}
	}
}
...
function sitemember_add_ui(){require_once('site-member/add.php');}
